...
export class LAppLive2DManager {
  ...
  public onTap(x: number, y: number): void {
    ...
    for (let i = 0; i < this._models.getSize(); i++) {
      // Attach the audio url to the audio element
      let filePath = "./src/audio/rsc_sample.wav";
      ...
      fetch(myRequest)
        ...
        .then((myBlob) => {
          ...
          // We will play the audio file only at the start only
          if (i == 0) {
            audio.play();
          }
          
          // Initialize lip sync for the model
